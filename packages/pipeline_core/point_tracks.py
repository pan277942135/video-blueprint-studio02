...
def run_point_track_refinement(
    ...
    *,
    ...
    """Track anonymous visual features inside certified E4.1 person masks.

    Point IDs are scoped to one character in one video. They are not biometric
    identifiers and are reset at shot boundaries. Missing/lost samples remain
    NaN/false; no temporal interpolation is performed.
    """
    config.validate()
        ...
        while decoded < frame_count:
            ...
            for character_id in ids:
                ...
                if int(np.count_nonzero(tids >= 0)) < minimum:
                    ...
                    seeds = seed_features(gray, mask, int(free.size), config)
                    for slot, point in zip(free, seeds, strict=False):
                        xy[slot] = point
                        tids[slot] = next_id[character_id]
                        next_id[character_id] += 1
    ...
    quality = {"coverage": round(minimum_coverage, 6), "score": round(minimum_coverage, 6), "confidence_available": False}
