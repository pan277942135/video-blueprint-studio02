...
def _check_ref(
    ...
    *,
    ...
    if not isinstance(ref, dict):
        return [f"E8 MicroMotion Contract Violation: {path} must be a TimeSeriesRef."]
    errors: list[str] = []
    expected = {
        "format": "npz",
        "dtype": "float32",
    ...
    for key, value in expected.items():
        if ref.get(key) != value:
            errors.append(f"E8 MicroMotion Contract Violation: {path}.{key} must equal {value!r}.")
    if not _sha256(ref.get("checksum_sha256")):
        errors.append(f"E8 MicroMotion Contract Violation: {path}.checksum_sha256 must be SHA256 hex.")
    metadata = ref.get("metadata")
    if not isinstance(metadata, dict):
        return errors + [f"E8 MicroMotion Contract Violation: {path}.metadata is required."]
    expected_metadata = {
        "array_key": array_key,
    ...
    for key, value in expected_metadata.items():
        if metadata.get(key) != value:
            errors.append(f"E8 MicroMotion Contract Violation: {path}.metadata.{key} must equal {value!r}.")
    return errors


def validate_micro_motion_contract(blueprint: dict[str, Any]) -> list[str]:
    extensions = blueprint.get("extensions")
    if not isinstance(extensions, dict) or "e8_micro_motion" not in extensions:
        return []
    extension = extensions.get("e8_micro_motion")
    path = "extensions.e8_micro_motion"
    if not isinstance(extension, dict):
        return [f"E8 MicroMotion Contract Violation: {path} must be an object."]
    errors: list[str] = []
    expected_extension = {
        "enabled": True,
        "algorithm": "body_local_sparse_periodicity_v1",
    ...
    if not _sha256(config_sha):
        errors.append(f"E8 MicroMotion Contract Violation: {path}.config_sha256 must be SHA256 hex.")
    thresholds = extension.get("thresholds")
    if not isinstance(thresholds, dict):
        return errors + [f"E8 MicroMotion Contract Violation: {path}.thresholds is required."]
    frame_count = blueprint.get("timebase", {}).get("frame_count")
    if not isinstance(frame_count, int) or isinstance(frame_count, bool) or frame_count <= 0:
        return errors + ["E8 MicroMotion Contract Violation: timebase.frame_count must be positive integer."]
    if not isinstance(extensions.get("e7_surface_motion"), dict):
        errors.append("E8 MicroMotion Evidence Violation: e7_surface_motion is required.")

    characters = blueprint.get("characters")
    if not isinstance(characters, list):
        return errors + ["E8 MicroMotion Contract Violation: characters must be an array."]
    rows = extension.get("characters")
    if not isinstance(rows, list) or len(rows) != len(characters):
        errors.append("E8 MicroMotion Contract Violation: extension character rows must match characters[].")
        rows = []
    ...
    for index, character in enumerate(characters):
        ...
        if not isinstance(character, dict):
            errors.append(f"E8 MicroMotion Contract Violation: {cpath} must be an object.")
            continue
        ...
        if not isinstance(surface, dict) or surface.get("coordinate_frame") != "body_local_2d":
            errors.append(f"E8 MicroMotion Evidence Violation: {cpath} requires body_local_2d surface motion.")
            continue
        ...
        if not isinstance(regions, list):
            errors.append(f"E8 MicroMotion Contract Violation: {cpath}.surface_motion.regions must be an array.")
            continue
        if not regions:
            if not isinstance(row, dict) or row.get("kind") != "not_detected" or row.get("signal_ref") is not None:
                errors.append(f"E8 MicroMotion Contract Violation: extension row for {character_id!r} must record no-region not_detected state.")
            continue
        if len(regions) != 1 or not isinstance(regions[0], dict):
            errors.append(f"E8 MicroMotion Contract Violation: {cpath} must contain exactly one E7 sparse region.")
            continue
        region = regions[0]
        ...
        if not isinstance(micro, dict):
            errors.append(f"E8 MicroMotion Contract Violation: {rpath}.micro_motion must be an object.")
            continue
        kind = micro.get("kind")
        if kind not in _ALLOWED_KINDS:
            errors.append(f"E8 MicroMotion Evidence Violation: {rpath}.micro_motion.kind is outside E8.1 geometry-only scope.")
        if micro.get("radial_expansion_ratio_ref") is not None or micro.get("area_change_ratio_ref") is not None:
            errors.append(f"E8 MicroMotion Evidence Violation: {rpath} cannot emit radial/area modes in E8.1.")
        smoothing = micro.get("smoothing")
        if smoothing != {"method": "none", "parameters": {}}:
            errors.append(f"E8 MicroMotion Contract Violation: {rpath}.micro_motion.smoothing must remain disabled.")
        ...
        if not isinstance(limitations, list) or not any(
            ...
            errors.append(f"E8 MicroMotion Evidence Violation: {rpath}.micro_motion.limitations must state the physiological inference prohibition.")
        if source_uri is None or body_uri is None:
            errors.append(f"E8 MicroMotion Evidence Violation: {rpath} requires physical E7 and E6 source refs.")
            continue
        refs = {
        ...
        if concrete_refs:
            ...
            if len(uris) != 1 or len(checksums) != 1:
                errors.append(f"E8 MicroMotion Contract Violation: {rpath} signal refs must share one physical NPZ.")
        if not isinstance(row, dict) or row.get("kind") != kind or row.get("signal_ref") != micro.get("signal_ref"):
            errors.append(f"E8 MicroMotion Cross-Reference Violation: extension row for {character_id!r} must match MicroMotion output.")

        usable = micro.get("usable_for_generation") is True
        if usable:
            if kind != "periodic_micro_motion" or micro.get("dominant_frequency_hz") is None:
                errors.append(f"E8 MicroMotion Quality Violation: {rpath} usable output must be periodic with a dominant frequency.")
            numeric_checks = (
            ...
            for metric, threshold_name, direction in numeric_checks:
                ...
                if not isinstance(metric_value, (int, float)) or isinstance(metric_value, bool):
                    errors.append(f"E8 MicroMotion Quality Violation: {rpath}.micro_motion.{metric} must be numeric.")
                    continue
                if not isinstance(threshold_value, (int, float)) or isinstance(threshold_value, bool):
                    errors.append(f"E8 MicroMotion Contract Violation: threshold {threshold_name} must be numeric.")
                    continue
                if direction == ">=" and float(metric_value) < float(threshold_value):
                    errors.append(f"E8 MicroMotion Quality Violation: {rpath}.micro_motion.{metric} is below usable threshold.")
                if direction == "<=" and float(metric_value) > float(threshold_value):
                    errors.append(f"E8 MicroMotion Quality Violation: {rpath}.micro_motion.{metric} exceeds usable threshold.")
            cycles = row.get("cycles_observed") if isinstance(row, dict) else None
            ...
            if not isinstance(cycles, (int, float)) or not isinstance(min_cycles, (int, float)) or float(cycles) < float(min_cycles):
                errors.append(f"E8 MicroMotion Quality Violation: {rpath} usable output must satisfy minimum observed cycles.")
    return errors
